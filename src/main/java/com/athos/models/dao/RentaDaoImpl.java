...
public class RentaDaoImpl implements IRentaDao{
    ...
    public List<Renta> getALL() {
        try
        {
            ...
            while (rs.next())
            {
                renta = new Renta(rs.getInt("id_renta"), rs.getString("fecha_renta"), rs.getString("fecha_devolucion"), rs.getInt("cliente_id"), rs.getInt("libro_id"), rs.getInt("sucursal_id"));
